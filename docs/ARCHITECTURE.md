...
- \__init_\_() - initialization, creates Stemmer and Parser instances
...
- prefixes - prefixes (за, на, по, при, от, до, с, у, в, вы, под, над, раз, рас, пере, про, об)
- suffixes - suffixes and endings (ться, тся, ти, ть, ет, ит, ат, ят, ут, ют, вш, вши, ш, ши, and others)
...
- URL: str - source website address
...
- parse_words_from_html(html) -> dict - parses HTML and extracts words with definitions

**src/bot/config.py - Configuration**  
Contains the BOT_TOKEN variable - the Telegram bot token obtained from BotFather.
